  ...
  "ReleaseTrain": {
      "mercury-21.02": {
        "DeployEnv": {
           ...
           "ProdStl"  : { "SchedStart":"", "SchedEnd":"", "CRQ":"", "CRQStatus":"", "ReleaseStatus":"" }
         }
      }
   }
}  
...
{
  "DOMAIN": "mercury",
  "LAST_SYSTEM_UPDATE": "02-13-2021 01:12",
  "RELEASE_TRAIN": {
    "mercury-21.02": {
      "DEPLOY_ENV": {
        "itf": {
...
```bash
cat temp-archer.json | jq 'del(."alerts"."product".nonprod."localhost.mastercard.int"[] | select( .  == "/tmp/app.jar"))'
```
...
ldapsearch -o ldif-wrap=no -x -LLL -h ldaps://stldc66.corp.mastercard.test -p 636 -b dc=corp,dc=mastercard,dc=test -D 'MCTEST\e040607' -w '$MCTESTPW' -s sub '(name=e040607)'
```
